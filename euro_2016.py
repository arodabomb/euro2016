...
def get_player_data():
	...
	table_1_ext = [[td.text for td in tr.find_all(['td', 'th'])] for tr in tables[0].find_all('tr')]
	tables_list = [[[td.text for td in tr.find_all(['td', 'th'])] for tr in table.find_all('tr')] for table in tables]
	data_df_list = [pd.DataFrame(table_1[1:], columns=table_1[0]) for table_1 in tables_list if table_1[0] == table_1_ext[0]]
	countries = [span.text for span in spans if u'Group' not in span.text][:-4]

	player_df = pd.DataFrame()

	for idx, table_1 in enumerate(tables):
		# print idx
		raw_list = [[td for td in tr.find_all(['td', 'th'])] for tr in table_1.find_all('tr')]
		
		ele_list = [[cell.text for cell in row] for row in raw_list]

		if ele_list[0] == columns:
			clubs = [raw_list_ele[-1].find('a').get('href').split('/')[-1] for raw_list_ele in raw_list[1:]]
			clubs = [re.sub("_", " ", ele) for ele in clubs]
			ele_df = pd.DataFrame(ele_list[1:], columns=columns)
			ele_df['Club.Country'] = clubs
			ele_df['Country'] = countries[idx]
			player_df = player_df.append(ele_df)
	return player_df
